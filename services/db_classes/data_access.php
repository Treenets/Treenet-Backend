...
abstract class Data_Access {
	...
	protected function dbConnect() {
		...
		define("CONST_DB_HOST", "localhost:3306");  // update with the location of your MySQL host.
	...
	//--------------------------------------------------------------------------------------------------------------------
	protected function getResultSetArrayPostgres($varQuery, $db) {

		$result = pg_query($db, $varQuery);
		if (pg_num_rows($result) > 0) {
			// add array to return
			$responseArray = App_Response::getResponse('200');
			$responseArray['dataArray'] = pg_fetch_all($result);
		}	else {
			// no data returned
			$responseArray = App_Response::getResponse('204');
			$responseArray['message'] = 'Query did not return any results.';
